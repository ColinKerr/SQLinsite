...
- `sqlinsite visualize` - Command to visualize `sqlinsite profile` and `sqlinsite map` results.  See details in [VISUALIZE.md](./commands/VISUALIZE.md)
  - `sqlinsite visualize serve` - Start a local web server hosting an interactive canvas view (runs in the foreground; Ctrl-C to stop).
    ...
    - `--profile-file` - Optional.  Path to the profile **SQLite database** generated by `sqlinsite profile`; imported as loaded ('input') sources of the unified profile overlay. Interactive Query-view runs are added as ('query') sources of the same overlay, so any profile can be shown in any view.
