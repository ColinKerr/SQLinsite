...
// The draggable divider between the left-hand Navigation Panel and the view.
// Width is shared (legendWidth) and persisted, so it's consistent across views.
export function PanelResizer() {
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      if (!dragging) return;
      const w = Math.max(140, Math.min(window.innerWidth * 0.6, e.clientX - 3));
      setLegendWidth(w);
    };
    const onUp = () => {
      if (!dragging) return;
