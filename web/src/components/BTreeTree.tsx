...
export function BTreeTree() {
  ...
  // only the chevron does); the shared selection highlight moves to the node; and
  // the active view's registered activation handler runs. The tree is view-agnostic
  // — it does not know what any view does with the node.
  const onRowClick = (node: TreeNode, isOpen: boolean) => {
    ...
    highlightNode(node);
    useTreeSelection.getState().activate(node);
  };
